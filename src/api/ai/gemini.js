...
// Construimos la URL dinámicamente usando la variable
...
const LOCAL_ANSWERS = {
    ...
    "¿Cómo reciclar vidrio?": "El vidrio es 100% reciclable. Lávalo, quita las tapas y sepáralo por colores si es posible. 🍾",
    "¿Qué es basura electrónica?": "Son cables, celulares viejos o computadoras. No los tires a la basura común, busca campañas de 'TecnoReciclaje'. 🖥️",
    "¿El tecnopor se recicla?": "Es difícil de reciclar. Lo mejor es evitar su uso y optar por envases biodegradables. 🥡",
    "Tips para principiantes": "1. Empieza con botellas plásticas. 2. Aplasta las latas. 3. Lava los envases antes de botarlos. ✨"
};

/**
 * Envía un mensaje a Gemini 1.5 Flash usando AXIOS o usa respuestas locales si falla
...
export const sendMessageToGemini = async (text) => {
    ...
    // 1. Revisar si tenemos respuesta local
    ...
    try {
        // 2. Petición HTTP con Axios usando la URL dinámica
...
Pregunta del usuario: ${cleanedText}`
        ...
        if (error.response) {
            console.error('❌ Error API Gemini:', error.response.status, error.response.data);
...
export const getSuggestedQuestions = () => {
    return Object.keys(LOCAL_ANSWERS);
};